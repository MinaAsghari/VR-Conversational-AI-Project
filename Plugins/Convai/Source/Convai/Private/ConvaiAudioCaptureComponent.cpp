...
void UConvaiAudioCaptureComponent::BeginDestroy()
{
	...
	// CRITICAL FIX: Force clear the ready-for-destroy flag BEFORE calling Stop()
	// During world teardown, the audio render thread may not process the stop properly
	// which would leave bIsNotReadyForForFinishDestroy = true, causing an infinite wait
	bIsNotReadyForForFinishDestroy = false;
	
	// Close the stream directly to avoid relying on OnEndGenerate callback
