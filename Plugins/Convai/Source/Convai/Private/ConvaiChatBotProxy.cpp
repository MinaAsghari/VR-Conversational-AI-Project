...
void UConvaiChatBotQueryProxy::Activate()
{
	...
	// Form Validation
	if (
		!UConvaiFormValidation::ValidateAuthKey(AuthKey) || !UConvaiFormValidation::ValidateInputText(UserQuery) || !UConvaiFormValidation::ValidateCharacterID(CharID) || !UConvaiFormValidation::ValidateSessionID(SessionID))
	{
		failed();
		return;
	}
...
void UConvaiChatBotQueryProxy::onHttpRequestComplete(FHttpRequestPtr RequestPtr, FHttpResponsePtr ResponsePtr, bool bWasSuccessful)
{
	...
	if (FJsonSerializer::Deserialize(Reader, JsonValue))
	{
		...
		BotText = JsonValue->AsObject()->GetStringField(JSON_FIELD("text"));
		NewSessionID = JsonValue->AsObject()->GetStringField(JSON_FIELD("sessionID"));
		if (VoiceResponse)
		{
			AudioContentString = JsonValue->AsObject()->GetStringField(JSON_FIELD("audio"));
			SampleRateStr = JsonValue->AsObject()->GetStringField(JSON_FIELD("sample_rate"));
		...
		if (Classification)
		{
			ClassifiedAction = JsonValue->AsObject()->GetStringField(JSON_FIELD("tag"));
		}
	}
	else
	{
...
void UConvaiChatBotQueryFromAudioProxy::Activate()
{
	...
	// Form Validation
	if (
		!UConvaiFormValidation::ValidateAuthKey(AuthKey) || !UConvaiFormValidation::ValidateInputVoice(Payload) || !UConvaiFormValidation::ValidateCharacterID(CharID) || !UConvaiFormValidation::ValidateSessionID(SessionID))
	{
		failed();
		return;
	}
...
void UConvaiChatBotQueryFromAudioProxy::onHttpRequestComplete(FHttpRequestPtr RequestPtr, FHttpResponsePtr ResponsePtr, bool bWasSuccessful)
{
	...
	if (FJsonSerializer::Deserialize(Reader, JsonValue))
	{
		...
		BotText = JsonValue->AsObject()->GetStringField(JSON_FIELD("text"));
		NewSessionID = JsonValue->AsObject()->GetStringField(JSON_FIELD("sessionID"));
		UserQuery = JsonValue->AsObject()->GetStringField(JSON_FIELD("UserQuery"));
...
void UConvaiChatBotCreateProxy::Activate()
{
	...
	// Form Validation
	if (
		!UConvaiFormValidation::ValidateAuthKey(AuthKey) || !UConvaiFormValidation::ValidateCharacterName(CharName) || !UConvaiFormValidation::ValidateBackstory(Backstory) || !UConvaiFormValidation::ValidateVoiceType(Voice))
	{
		failed();
		return;
	}
...
void UConvaiChatBotCreateProxy::onHttpRequestComplete(FHttpRequestPtr RequestPtr, FHttpResponsePtr ResponsePtr, bool bWasSuccessful)
{
	...
	{
		// Get the value of the json object by field name
		CharID = JsonValue->AsObject()->GetStringField(JSON_FIELD("charID"));
		if (CharID.Len() == 0)
		{
...
void UConvaiChatBotUpdateProxy::Activate()
{
	...
	// Form Validation
	if (
		!UConvaiFormValidation::ValidateAuthKey(AuthKey) || !UConvaiFormValidation::ValidateCharacterID(CharID)
...
void UConvaiChatBotUpdateProxy::onHttpRequestComplete(FHttpRequestPtr RequestPtr, FHttpResponsePtr ResponsePtr, bool bWasSuccessful)
{
	...
	{
		// Get the value of the json object by field name
		FString status = JsonValue->AsObject()->GetStringField(JSON_FIELD("STATUS"));
		if (status.Len() == 0)
		{
...
void UConvaiChatBotGetDetailsProxy::onHttpRequestComplete(FHttpRequestPtr RequestPtr, FHttpResponsePtr ResponsePtr, bool bWasSuccessful)
{
	...
	{
		// Get the value of the json object by field name
		character_name = JsonValue->AsObject()->GetStringField(JSON_FIELD("character_name"));
		// user_id			= JsonValue->AsObject()->GetStringField(JSON_FIELD("user_id");
		FString character_id = JsonValue->AsObject()->GetStringField(JSON_FIELD("character_id"));
		voice_type = JsonValue->AsObject()->GetStringField(JSON_FIELD("voice_type"));
		// timestamp		= JsonValue->AsObject()->GetStringField(JSON_FIELD("timestamp");
		backstory = JsonValue->AsObject()->GetStringField(JSON_FIELD("backstory"));
		LanguageCode = JsonValue->AsObject()->GetStringField(JSON_FIELD("language_code"));
...
void UConvaiChatBotGetCharsProxy::onHttpRequestComplete(FHttpRequestPtr RequestPtr, FHttpResponsePtr ResponsePtr, bool bWasSuccessful)
{
	...
	if (FJsonSerializer::Deserialize(Reader, JsonValue))
	{
		...
		if (JsonValue->AsObject()->TryGetArrayField(JSON_FIELD("characters"), OutArray))
		{
			for (int i = 0; i < OutArray->Num(); i++)
			{
				FString CharID;
				if ((*OutArray)[i]->AsObject()->TryGetStringField(JSON_FIELD("character_id"), CharID))
				{
					CharIDs.Add(CharID);
				}
				else
...
void UConvaiDownloadImageProxy::Activate()
{
	...
	if (URL.EndsWith("/"))
	{
		...
		URL.RemoveAt(URL.Len() - 1, 1, EAllowShrinking::No);
#else
...
FString UConvaiGetAvailableVoicesProxy::GetLanguageCodeFromEnum(ELanguageType LanguageType)
{
	...
	case ELanguageType::Arabic:
		return "ar";
	case ELanguageType::ChineseCantonese:
		return "zh";
	case ELanguageType::ChineseMandarin:
		return "zh";
	case ELanguageType::Dutch:
		return "nl";
	case ELanguageType::DutchBelgium:
		return "nl";
	case ELanguageType::English:
		return "en";
	case ELanguageType::Finnish:
		return "fi";
	...
	case ELanguageType::Hindi:
		return "hi";
	case ELanguageType::Italian:
		return "it";
	case ELanguageType::Japanese:
		return "ja";
	case ELanguageType::Korean:
		return "ko";
	case ELanguageType::Polish:
		return "pl";
	case ELanguageType::PortugueseBrazil:
		return "pt";
	case ELanguageType::PortuguesePortugal:
		return "pt";
	case ELanguageType::Russian:
		return "ru";
	case ELanguageType::Spanish:
		return "es";
	case ELanguageType::SpanishMexico:
		return "es";
	case ELanguageType::SpanishUS:
		return "es";
	case ELanguageType::Swedish:
		return "sv";
	case ELanguageType::Turkish:
		return "tr";
	case ELanguageType::Vietnamese:
		return "vi";
	default:
		return "";
	}
